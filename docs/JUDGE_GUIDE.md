...
- **A 选项（推荐）公网 Demo**
  - 主页：[http://118.178.242.26/](http://118.178.242.26/) — 全新动画首页
  - 仪表盘：[http://118.178.242.26/dashboard](http://118.178.242.26/dashboard)
  - 多端协同：[http://118.178.242.26/multi-end](http://118.178.242.26/multi-end)
  - API 文档：[http://118.178.242.26/docs](http://118.178.242.26/docs)
  - MCP 反向暴露：`http://118.178.242.26/mcp/`（接 Cursor / Claude / Trae）
  - 健康：[http://118.178.242.26/health](http://118.178.242.26/health) → `{"status":"healthy","version":"v1.0.0"}`
...
实时进度：http://118.178.242.26/dashboard?plan_id=plan_xxx
```
...
打开两个端：
1. 手机飞书 IM 中触发任务
2. 电脑浏览器打开 [http://118.178.242.26/multi-end](http://118.178.242.26/multi-end)
...
```bash
# 列出 V1 提供的工具
curl http://118.178.242.26/mcp/tools/list

# 反向调用 doc.create
curl -X POST http://118.178.242.26/mcp/tools/call \
...
[http://118.178.242.26/](http://118.178.242.26/) 主页含 5 层架构 + 8 步 loop 动画图。

```bash
# 看工具清单（按 read/write 分类）
curl http://118.178.242.26/api/tools

# 看实时 sessions
curl http://118.178.242.26/api/sessions
```
