...
def task_suggested_card(
    *,
    ...
    btns = [
        ...
        _button("📎 添加资料", action="pilot.task.add_context", value={"task_id": task_id}, eid="pilot.btn.add_ctx"),
        _button("👤 指派他人", action="pilot.task.assign", value={"task_id": task_id}, eid="pilot.btn.assign"),
    ]
    if detail_url:
        btns.append(_button("🔍 查看详情", url=detail_url, eid="pilot.btn.detail"))
    btns.append(
        _button(
            "🙈 忽略", action="pilot.task.ignore", value={"task_id": task_id}, style="danger", eid="pilot.btn.ignore"
        )
    )
    body.append(
        {
...
def assign_picker_card(
    *,
    ...
    if current_owner_open_id:
        body.append(_text(f"_当前 owner: `{current_owner_open_id[-6:]}`_", eid="pilot.assign.curr"))
    body.append(_divider())
    ...
    body.append(
        {
            ...
            "actions": [
                ...
                _button(
                    "取消", action="pilot.task.assign_cancel", value={"task_id": task_id}, eid="pilot.assign.btn.cancel"
                ),
            ],
...
def context_confirm_card(
    *,
    ...
    body: List[Dict[str, Any]] = [
        _text(f"**📦 即将使用的资料**  ·  task: `{task_id[-8:]}`", eid="pilot.ctx.header"),
    ...
    if summary.get("must_cite"):
        body.append(_text("✓ 启用 Citation Agent · 每条 claim 自动标 source", eid="pilot.ctx.cite"))
    body.append(_divider())
    ...
    body.append(
        {
            ...
            "actions": [
                _button(
                    ("✅ 确认生成" if can_proceed else "⚠ 信息不足，仍要生成"),
                ...
                _button(
                    "📎 继续添加资料", action="pilot.ctx.add_more", value={"task_id": task_id}, eid="pilot.ctx.btn.add"
                ),
                _button("⏸ 暂停", action="pilot.task.pause", value={"task_id": task_id}, eid="pilot.ctx.btn.pause"),
            ],
...
def multi_agent_card(
    *,
    ...
    body: List[Dict[str, Any]] = [
        _text(
            f"**🤖 多 Agent 协同**  ·  task: `{task_id[-8:]}`  ·  pipeline: `{pipeline_id[-8:]}`", eid="pilot.ma.header"
        ),
        _divider(),
...
def task_progress_card(
    *,
    ...
    body: List[Dict[str, Any]] = [
        _text(f"**🛫 任务执行**  ·  task: `{task_id[-8:]}`  ·  状态：{state}", eid="pilot.prog.header"),
...
def task_delivered_card(
    *,
    ...
    actions = []
    if share_url:
        actions.append(_button("🔗 分享链接", url=share_url, eid="pilot.done.btn.share", style="primary"))
    actions.append(
        _button("📦 归档", action="pilot.task.archive", value={"task_id": task_id}, eid="pilot.done.btn.archive")
    )
    actions.append(
        _button(
            "🔄 继续生成 PPT", action="pilot.task.request_ppt", value={"task_id": task_id}, eid="pilot.done.btn.ppt"
        )
    )
...
def task_clarify_card(
    *,
    ...
    body: List[Dict[str, Any]] = [
        _text(f"**🤔 Agent 需要更多信息**  ·  task: `{task_id[-8:]}`", eid="pilot.clar.header"),
    ]
    if detected_goal:
        body.append(_text(f"识别到的初步目标：「{detected_goal[:100]}」", eid="pilot.clar.goal"))
    body.append(_divider())
    ...
    body.append(
        {
            ...
            "actions": [
                _button(
                    "✏️ 在对话中回答",
                    action="pilot.task.clarify_inline",
                    value={"task_id": task_id},
                    style="primary",
